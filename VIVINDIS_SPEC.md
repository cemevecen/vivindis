...
- **`next-intl`** — `src/i18n/routing.ts`, `src/i18n/request.ts`, `src/messages/*.json`, URL **`/{locale}/...`** (varsayılan **tr**), `src/middleware.ts` (Clerk + locale). Kayıtlı mağaza uygulamaları listesi: **yerelleştirilmiş URL** (`pathnames`, ör. `tr` → `/tr/kayitlar`, `en` → `/en/records`); iç route dosyaları `app/.../apps/*`; eski **`/{locale}/apps/...`** istekleri **308** ile yeni yola yönlendirilir.
- **Oturum 6 — dashboard & kayıtlar (liste):** TanStack Query ile `GET /api/v1/apps`, `POST /api/v1/apps`, uygulama detayı `GET/POST .../apps/{id}/fetch`; **react-hook-form + zod** (`/apps/new` iç route); `AppCard` / liste / skeleton / boş durum; Clerk yoksa bilgilendirici panel
- **Oturum 7 — analiz:** `apps/[id]/analysis?fetchId=…` (dış URL locale’e göre, ör. `/tr/kayitlar/{id}/analysis`), **`GET /api/v1/apps/{app_id}/fetches/{fetch_id}`** (tekil fetch), fetch + analiz satırları için **~3 sn `refetchInterval`**; **Recharts** (duygu pasta, puan bar, konu bar); `POST /api/v1/fetches/{id}/analyze`
- **Oturum 8 — polish:** dashboard **mobil drawer** (`DashboardShellClient`, backdrop, `md:` masaüstü); **`EmptyState`** + **`/compare`** placeholder (CTA → kayıtlar listesi, iç `href` `/apps`); **Sonner** (`closeButton`, süre, toast sınıfları); **`compare.*` / `navigation.sidebarNav` / `common.toastNetwork`** 12 dilde
- **Analiz merkezi (modül):** `components/analyze/analyze-hub.tsx` (Clerk’lı akış, sekme durumu, sorgular); `analyze-hub-parts.tsx` (`SegmentedTwo`, `StoreResultCard`); `lib/analyze-hub-utils.ts` (tarih aralığı, mağaza→uygulama gövdesi, **`analyzeStoreSourceFromPathname`**: `/{locale}/analyze/store` vs `/{locale}/analyze/marketplace`). Rotalar: **`/analyze/store`**, **`/analyze/marketplace`**, diğer sekmeler **`/analyze?mode=file|text|compare`**; `app/[locale]/(dashboard)/analyze/page.tsx` mağaza modunda kanonik `/analyze/store` yönlendirmesi. Dosya / yapıştırma: `lib/parse-review-file.ts` + `lib/review-import-parse.ts`. İki uygulama özeti: `components/compare/compare-apps-dashboard.tsx`, rota **`/compare?app_a=&app_b=`**; statik **Hakkında:** `app/[locale]/(dashboard)/about/page.tsx`. Kök hata sınırları: `app/error.tsx`, `app/global-error.tsx`; yerel önbellek temizliği: `npm run clean` / `npm run dev:clean`.
