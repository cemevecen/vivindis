...
def generate_reviews_one_shot_json(
    reviews_payload: str,
    *,
    max_retries: int = 3,
) -> dict[str, Any]:
    ...
    prompt = f"""You are analyzing mobile app user reviews in one shot.
Return ONLY a JSON object with these keys (use numbers/strings consistently):
- "overall_score": float 0-10
- "sentiment": {{"positive":0-1,"neutral":0-1,"negative":0-1}}
- "rating_distribution": {{"1":n,"2":n,"3":n,"4":n,"5":n}}
- "top_topics": [{{"topic":str,"count":int,"sentiment":"positive"|"neutral"|"negative"}}] max 15
- "top_issues": [{{"issue":str,"count":int,"severity":"high"|"medium"|"low"}}] max 12
- "highlights": [{{"type":"positive"|"negative","text":str,"review_id":str}}] max 12
- "recommendations": [str] max 12 short strings
- "lang_distribution": {{lang_code: proportion}}
