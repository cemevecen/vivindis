  ...
  const fetchRowQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (err instanceof ApiError && err.status === 404 && recoveryAppId && storeFetchId) {
          const rows = await apiFetch<ReviewFetchDto[]>(`/api/v1/apps/${recoveryAppId}/fetches`, { getToken });
